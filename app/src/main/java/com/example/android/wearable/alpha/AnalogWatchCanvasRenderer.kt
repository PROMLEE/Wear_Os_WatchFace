...
class AnalogWatchCanvasRenderer(
    ...
    private val rolex1: Bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.preview_analog)
    override fun render(
        canvas: Canvas,
        bounds: Rect,
        zonedDateTime: ZonedDateTime,
...
//        canvas.drawColor(backgroundColor)
        canvas.drawBitmap(rolex1, null, bounds, null)
    ...
    private val rolexhour1: Bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.watch_min)
    private val rolexmin1: Bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.watch_min)
    private val rolexsec1: Bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.watch_min)
    private val rolexcenter1: Bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.white_style)
    ...
    private fun drawClockHands(
        ...
        val X : Int = bounds.exactCenterX().toInt()
        val Y : Int = bounds.exactCenterX().toInt()
        var between : Int = 0
        ...
        canvas.withScale(
            ...
            withRotation(hourRotation, bounds.exactCenterX(), bounds.exactCenterY()) {
                var length: Int = 100
                var width : Int = 15
                drawBitmap(rolexhour1,null,Rect(X-width,Y-between-length,X+width,Y-between),null)
            }
...
//            val bottom = centerY - gapBetweenHandAndCenter * width
            withRotation(minuteRotation, bounds.exactCenterX(), bounds.exactCenterY()) {
                var length: Int = 180
                var width : Int = 8
                drawBitmap(rolexmin1,null,Rect(X-width,Y-between-length,X+width,Y-between),null)
            }
            ...
            if (!drawAmbient) {
                ...
                withRotation(secondsRotation, bounds.exactCenterX(), bounds.exactCenterY()) {
                    var length: Int = 180
                    var width : Int = 3
                    drawBitmap(rolexsec1,null,Rect(X-width,Y-between-length,X+width,Y-between),null)

                }
                var centersize:Int = 25
                drawBitmap(rolexcenter1,null,Rect(X-centersize,Y-centersize,X+centersize,Y+centersize),null)
            }
        }
    }
